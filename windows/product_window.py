...
class ProductWindow(QMainWindow):
    ...
    def load_products(self):
        ...
        for row, prod in enumerate(products):
            ...
            self.table.setItem(row, 4, QTableWidgetItem(short_desc or ""))
            ...
            label_price.setTextFormat(Qt.RichText)  # разрешаем HTML
            ...
            if self.role == 'Администратор':
                ...
                edit_btn.clicked.connect(lambda checked, a=article: self.edit_product(a))
    ...
    def add_product(self):
        if self.edit_dialog and self.edit_dialog.isVisible():
            ...
            return
        self.edit_dialog = ProductEditDialog(self, article=None)
    ...
    def edit_product(self, article):
        if self.edit_dialog and self.edit_dialog.isVisible():
            QMessageBox.warning(self, "Внимание", "Окно редактирования уже открыто.")
            return
        self.edit_dialog = ProductEditDialog(self, article=article)
        self.edit_dialog.finished.connect(self.on_edit_finished)
        self.edit_dialog.show()

    def edit_product_from_table(self, index):
        row = index.row()
        article_item = self.table.item(row, 1)
        if article_item:
            self.edit_product(article_item.text())

    def delete_product(self, article):
        if product_exists_in_orders(article):
            QMessageBox.warning(self, "Невозможно удалить",
                                f"Товар с артикулом {article} присутствует в заказах.\nУдаление запрещено.")
            return
        reply = QMessageBox.question(self, "Подтверждение удаления",
                                     f"Вы действительно хотите удалить товар {article}?",
